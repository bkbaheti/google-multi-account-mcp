...
- **feat:** `calendar_create_event` and `calendar_update_event` accept `addMeet: true`, which generates a new Google Meet conference and attaches it. Because Google creates conferences asynchronously, an insert that comes back `pending` is re-read once. That closes the common case; a conference still pending after the re-read is returned with its `pending` status rather than as a silently linkless event. If that read fails, the successful create is still returned — reporting it as an error would invite a retry, and a second event with a second round of invitations.
...
- **feat:** `calendar_list_calendars` reports `canEdit` (derived from `accessRole`, counting `writerWithoutPrivateAccess` as writable — Google documents five roles, and that one grants write access while hiding private event details) so a shared calendar you can only read is distinguishable from one you can write to, plus `summaryOverride` — the user's own rename of a shared calendar, often the only name they recognise — and `selected`/`hidden`/`deleted`. It now paginates (`maxResults`, `pageToken`, `nextPageToken`) and takes `showHidden`/`showDeleted`; previously it passed no parameters and dropped `nextPageToken`, so an account subscribed to more than 100 calendars silently lost the tail.
- **fix:** `calendar_update_event` sends `events.patch` instead of `events.update`. Update is full replacement, so every update was a read-modify-write that lost any concurrent edit landing between the read and the write; patch changes only the fields the caller named. Because patch merges nested objects, `start`/`end` explicitly null the variant they replace — without that, converting an event between all-day and timed leaves both `date` and `dateTime` set and Google rejects it.
- **BREAKING:** `calendar_list_calendars` returns `{ calendars, nextPageToken? }` rather than a bare array — a caller indexing the result directly needs to read `.calendars`. `CalendarClient.listCalendars()` changes the same way for library consumers, and takes an options object. No MCP tool response shape changed for a caller reading `calendars`, but the tool result is now an object rather than an array.
...
Update notifications on `calendar_update_event` now always use `sendUpdates: 'all'`. This matches what the previous code already did for any event with guests, and an event without guests has nobody to notify — so the practical change is that the decision no longer requires reading the event first.
