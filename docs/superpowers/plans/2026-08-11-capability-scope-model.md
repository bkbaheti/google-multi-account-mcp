...
### Task 8: Pin every gate's capability with a test

**Files:**
- Create: `tests/unit/gate-mapping.test.ts`

**Interfaces:**
- Consumes: `registerDriveTools`, `registerCalendarTools`, `registerGmailTools`; `Capability` from `src/auth/capabilities.js`.
- Produces: nothing consumed by later tasks.

**Why this exists.** The Task 3 review found that no test exercises any gate — `drive-tools.test.ts` covers only the pure `normalizeDriveQuery` helper. Every gate could be mis-mapped and the suite would stay green, re-opening the exact bug this plan fixes. This task pins the tool-to-capability mapping so a future edit that weakens a gate fails loudly.

The technique: pass a fake `McpServer` that records `registerTool` calls, and a fake gate that records which capability each handler asks for, then invoke each handler once.
...
describe('Calendar gate mapping', () => {
  ...
  // calendar.events authorizes neither calendarList.list nor freebusy.query.
  it.each(['calendar_list_calendars', 'calendar_freebusy'])(
    '%s requires calendar:read',
    (tool) => {
  ...
  it.each(['calendar_list_events', 'calendar_get_event', 'calendar_search_events'])(
    '%s accepts either calendar capability',
    (tool) => {
...
Run: `pnpm vitest run tests/unit/gate-mapping.test.ts`

If it fails because the fakes do not match the real `registerTool` / gate signatures, adjust the fakes until the harness works — that is expected iteration. It must NOT be made to pass by loosening an assertion. Every expected capability above is verified against Google's per-method scope lists; if a real gate disagrees, the gate is wrong, not the test.

- [ ] **Step 3: Confirm the test actually bites**

Temporarily change one gate in `src/server/drive-tools.ts` — `drive_get_comments` from `'drive:read'` to `'drive:appfiles'` — and re-run. The test MUST fail. Revert the change. A pinning test that cannot detect the regression it exists to prevent is worthless; do not skip this step, and record its output in your report.

- [ ] **Step 4: Verify and commit**

```bash
pnpm test && pnpm typecheck && pnpm biome check tests/unit/gate-mapping.test.ts
git add tests/unit/gate-mapping.test.ts
git commit -m "test: pin every Drive and Calendar tool to its required capability"
```
